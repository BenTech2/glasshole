...
/**
 * RTMP broadcaster. Camera2 frames → H.264 + AAC encoder → RTMP push,
 * all driven by RtmpCamera2. We bind to the phone-side BroadcastPlugin
 * on open (START broadcast), get a CONFIG back with the URL + quality
 * settings, then start the stream.
 *
 * Display modes (per phone config):
 *   viewfinder   — preview visible, screen stays lit
 *   preview_off  — preview covered by a black view; screen stays lit
 *   screen_off   — window brightness driven to 0 after a short grace
 *                   so the display panel powers down; encoder keeps
 *                   running because the activity is still resumed.
 */
class BroadcastActivity : Activity() {
    ...
    // Gesture tracking. dispatchTouchEvent runs before both the
    // OpenGlView and the chat ScrollView can consume events.
    private var downX = 0f
    private var downY = 0f
    private var lastX = 0f
    private val horizScrollFactor = 2.5f

    // Auto-scroll-to-newest latch for the chat overlay. Goes false when the
    // user manually scrolls off the bottom; tap or scroll-back-to-bottom
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        camera = RtmpCamera2(surface, connectionListener)
    ...
    private fun tryStart() {
        ...
        try {
            val audioOk = if (cfg.audio) cam.prepareAudio() else cam.prepareAudio(
                64 * 1024, 32_000, false, false, false
            )
    ...
    private fun applyDisplayMode(mode: String) {
        when (mode) {
            "preview_off" -> {
                ...
                setScreenBrightness(-1f) // system default
            }
            "screen_off" -> {
                previewCover.visibility = View.VISIBLE
                chatScroll.visibility = View.GONE
            ...
            "chat" -> {
                // Cover the camera preview so the encoder surface can keep
                // rendering underneath while the viewer sees the chat feed.
                previewCover.visibility = View.VISIBLE
    ...
    override fun onTouchEvent(event: MotionEvent?): Boolean {
        // Tap wakes the display back up if we've dimmed for screen-off mode
    ...
    override fun dispatchTouchEvent(event: MotionEvent): Boolean {
        ...
        when (event.action) {
            MotionEvent.ACTION_DOWN -> {
                ...
                downY = event.y
                lastX = event.x
                // Wake the display back up on first touch when screen-off
                // dimming has kicked in.
                setScreenBrightness(-1f)
            ...
            MotionEvent.ACTION_UP -> {
                ...
                // Swipe down to close — wins in any display mode
    ...
    private fun acquireWakeLock() {
        try {
            ...
            wl.acquire(30 * 60 * 1000L) // 30 min cap; re-acquired on restart
            wakeLock = wl
        } catch (e: Exception) {
