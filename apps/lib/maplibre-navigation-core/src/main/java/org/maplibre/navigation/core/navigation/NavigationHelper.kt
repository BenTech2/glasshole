...
object NavigationHelper {
    ...
    fun stepDistanceRemaining(
        ...
        if (distance > 1) {
            Log.d("MapLibre", "Distance to step is larger than 1km, so we won't advance the step, distance: $distance km" )
    ...
    fun decodeStepPoints(
        ...
        return directionsRoute.legs
            ...
            ?.getOrNull(stepIndex)
            ?.let { step ->
                // 6.0.1's PolylineUtils returns Java Points; wrap each
                // into our shim Point so List<Point> matches the rest
                // of nav-core's shim-Point type.
