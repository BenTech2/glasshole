...
class DeviceHandler(
    ...
    private fun handleSleepNow() {
        try {
            // Use isScreenOn (API 7+, deprecated but universal) instead of
            // isInteractive (API 20+). Dalvik on EE1 (API 19) soft-fails
            // method verification when it sees an unresolved virtual method
            // reference and replaces the whole method body with
            // throw_verification_error — even if the runtime SDK_INT branch
            // would never call it.
            @Suppress("DEPRECATION")
            val interactive = power.isScreenOn
