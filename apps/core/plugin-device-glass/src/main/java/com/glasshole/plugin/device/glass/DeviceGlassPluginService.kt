...
class DeviceGlassPluginService : GlassPluginService() {
    ...
    /**
     * Force the glass to standby. Android 8+ (EE2) blocks direct
     * PowerManager.goToSleep via reflection, so we momentarily drop the
     * system's SCREEN_OFF_TIMEOUT to 1s (while the calling activity has
     * already finished), let the screen time out almost immediately, and
     * then restore the original value. Needs WRITE_SETTINGS — granted via
     * the Device Access tile on first run.
     */
    private fun handleSleepNow() {
        try {
            ...
            Settings.System.putInt(
                contentResolver, Settings.System.SCREEN_OFF_TIMEOUT, 1000
            )
            Log.i(TAG, "SLEEP_NOW — SCREEN_OFF_TIMEOUT $current → 1000 (temporary)")
            android.os.Handler(android.os.Looper.getMainLooper()).postDelayed({
                try {
                    Settings.System.putInt(
                        contentResolver, Settings.System.SCREEN_OFF_TIMEOUT, current
                    )
                    Log.i(TAG, "SCREEN_OFF_TIMEOUT restored → $current")
                } catch (e: Exception) {
