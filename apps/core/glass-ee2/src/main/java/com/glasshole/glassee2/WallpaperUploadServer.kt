...
class WallpaperUploadServer(
    ...
    companion object {
        ...
        /** 5 MB default. Comfortably bigger than any reasonable
         *  wallpaper / notif-sound clip but well below Glass's
         *  free-space cliff. APK installs override to a larger cap. */
        const val DEFAULT_MAX_SIZE_BYTES = 5L * 1024 * 1024
        /** Back-compat alias for the original constant name — callers
         *  outside the APK-install path still refer to this. */
        const val MAX_SIZE_BYTES = DEFAULT_MAX_SIZE_BYTES
        /** APK uploads — base app + plugins fit comfortably below
         *  100 MB even with native deps; pick the simplest round
         *  number that doesn't surprise us in a future plugin. */
