 ...
 * Notification card in Home. Oldest active notification first.
 *
 * Gestures:
 *   swipe forward / back  → next / previous notification page
 *   tap                   → show per-notification options overlay
 *     in overlay:
 *       swipe fwd / back  → cycle between "Dismiss" and "Cancel"
 ...
 *
 * Dismiss fires NOTIF_DISMISS over BT — phone cancels the system
 * notification, which triggers NOTIF_REMOVED coming back and updates
 * NotificationStore. We don't remove optimistically; the listener
 * fires fast enough that the drawer stays in sync.
 */
class NotificationDrawerActivity : Activity() {
    ...
    companion object {
        ...
        /** Page 0 is the virtual "Dismiss all" card — swiping left from
         *  the newest notification lands here. Actual notifications start
         *  at page 1. */
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (adapter.itemCount == 0) {
            // Nothing to show — bail back to Home rather than leave the
    ...
    private fun refreshFromStore() {
        // Newest first — most recent notification shows on page 1 (page 0
        // is the virtual "Dismiss all" card).
    ...
    private fun showOverlay() {
        ...
        overlayOptions = if (position == 0) {
            // Dismiss-all card: two options, Dismiss all + Cancel.
            listOf(
            ...
            // Same shape as the popup's overlay: the notification's own
            // actions first, then a synthetic Dismiss at the end.
    ...
    private fun executeOverlaySelection() {
        ...
        if (option.id == SYNTH_DISMISS_ALL_ID) {
            // Dismiss every active notification — fire NOTIF_DISMISS per
            // key for the phone (so system notifications get cancelled),
            // and drop the local store optimistically so the drawer
        ...
        if (option.id == SYNTH_DISMISS_ID) {
            // Optimistic removal: drop from the store immediately so the
            // drawer reflects the action even when the phone can't cancel
            // the underlying system notification (debug test keys, system-
            // locked notifs, etc.). If the phone DOES cancel, NOTIF_REMOVED
            // comes back and removes what we've already removed — harmless.
    ...
    // Page adapter: page 0 is a virtual "Dismiss all" card, pages 1..N
    // are the notifications in newest-first order.
    private class NotifPageAdapter : RecyclerView.Adapter<PageHolder>() {
        ...
        override fun onBindViewHolder(holder: PageHolder, position: Int) {
            if (position == 0) return  // static dismiss-all layout, nothing to bind
            ...
            if (e.pictureBitmap != null) {
                ...
                // Match the popup's picture layout: top spacer pushes text
                // down so title + body sit over the dark gradient.
                ...
                // Plain text: title at top, middle spacer pushes hint +
