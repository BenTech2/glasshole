...
class MainActivity : Activity(), BluetoothListenerService.MessageListener {
    ...
    /**
     * EE2 doesn't have the GDK timeline's built-in "swipe-down puts the
     * display to sleep" behavior. Fake it by asking the device plugin to
     * briefly drop SCREEN_OFF_TIMEOUT so the launcher times out right away.
     * Needs the WRITE_SETTINGS grant that Device Access provides.
     */
